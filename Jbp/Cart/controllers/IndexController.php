<?php
class Jbp_Cart_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function testAction(){
        ...
        $item = Mage::getSingleton('checkout/session')->getQuote()->getAllItems()[0];
        
        unset($item['item_id']);
        
        $qty = $item->getData('qty') - self::LIMIT_CART;
        ...
        echo "<pre>";
        var_dump($quote->getData('items_qty'));        
    }
    ...
    public function checkCartAction(){
         ...
         //$this->nextItem($posCurrent)->getOrderedQty()
    ...
    public function indexAction(){
        ...
        foreach($this->newCart as $cart){
            echo '<br/>Qty cart: '.$cart->getData('items_qty').' - ID: '.$cart->getEntityId().' - Total: '.$cart->getSubtotal().'<br/>';
            $rt += $cart->getSubtotal();
            $sh += $cart->getShippingAddress()->getData('shipping_amount');
            foreach($cart->getAllItems() as $item){
                echo $item->getName().' - '.$item->getQty().' - Price: '
                    .$item->getPrice().' Clone: '.$item->getCloneIs()."<br />";                
            }
            echo "<br/>-----------------------------------<br/>";
...
//             echo $increment_id.";";
    ...
    protected function generated(){        
        ...
        if($qtyTotalCart > self::LIMIT_CART){
            ...
            echo $this->newCart[1]->getAllItems()[0]->getPrice();
            echo "<br>";
            echo $this->newCart[1]->getAllItems()[0]->getName();
            echo "<br>";
            echo $cart->getId();
